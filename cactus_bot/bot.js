...
const antiMassMention = require("./database/antiMassMention.js")
const antiAlt = require("./database/antiAlt.js")
const antiSpam = require("./database/antiSpam.js")
...
let data = await antiAlt.findOne({
    Guild: member.guild.id
  })
  ...
  if(!antiAlt) return;
    ...
    Guild: member.guild.id
       ...
       if(data.Difficulty === "1"){
         if (Date.now() - member.user.createdAt < 1000*60*60*24*3) {
            ...
            if (regex.test(arg1)) {
                ...
                arg1 = "\"" + arg1 + "\"";
                args.push(arg1);
                replyMessage = getTimeSolid(args, eph, arg1);


            member.send(`Your account is under whats required to join the server! Please join back in ${replyMessage}`)    
            setTimeout(() => { member.kick("Anti Alt: Under 3 Days!") }, 1000);
       ...
       if(data.Difficulty === "2"){
         if (Date.now() - member.user.createdAt < 1000*60*60*24*6) {
            let arg1 = "6 days";
            let arg2 = "f";
            //let arg3 = interaction.options.getString('timezone');
            let args = [];
            ...
            member.send(`Your account is under whats required to join the server! Please join back in ${replyMessage}`)    
            setTimeout(() => { member.kick("Anti Alt: Under 6 Days!") }, 1000);
        ...
        if(data.Difficulty === "3"){
         if (Date.now() - member.user.createdAt < 1000*60*60*24*20) {
            ...
            member.send(`Your account is under whats required to join the server! Please join back in ${replyMessage}`)    
            setTimeout(() => { member.kick("Anti Alt: Under 20 Days!") }, 1000);
        ...
        if(data.Difficulty === "4"){
         if (Date.now() - member.user.createdAt < 1000*60*60*24*30) {
            ...
            member.send(`Your account is under whats required to join the server! Please join back in ${replyMessage}`)    
            setTimeout(() => { member.kick("Anti Alt: Under 30 Days!") }, 1000);
        ...
        if(data.Difficulty === "5"){
         if (Date.now() - member.user.createdAt < 1000*60*60*24*365) {
            ...
            member.send(`Your account is under whats required to join the server! Please join back in ${replyMessage}`)    
            setTimeout(() => { member.kick("Anti Alt: Under 365 Days!") }, 1000);
...
client.on('messageCreate', async(message) => {
  ...
  if(!message.guild) return;
    ...
    Guild: message.guild.id
      ...
      const ConvertedAmount = parseInt(data.Amount);
      
      
      
      const ConvertedTime = ms(data.Time)
      
      
     
      
      
      const ConvertedAmount2 = parseInt(ConvertedTime);
      
     
      
const LIMIT = ConvertedAmount;
const DIFF = ConvertedAmount2; 
    ...
    if(usersMap.has(message.author.id)) {
        ...
        if(difference > DIFF) {
            ...
            if(parseInt(msgCount) === LIMIT) {
              ...
              message.author.send({content: `It seems like your message has been removed for Spam? If your message didn't deserve to be called Spam and its just a mistake, here is your message back! \n` + "```\n" + `${message.content}` + "\n```", })
              
              
              
               message.reply(`Spamming is not allowed is not allowed in this server! ${message.author}`).then(msg => {
...
client.on('messageCreate', async(message) => {
    ...
    const data = await antiMassMention.findOne({ Guild: message.guild.id })
    if(!data) return;
  
    if(message.member.permissions.has("ADMINISTRATOR")) return;
  
    antiMassMention.findOne({ Guild: message.guild.id }, async(err, data) => {
        if((message.content.match(/<@/g) || []).length >= parseInt(data.Amount)){
            message.delete();
      
            return message.channel.send({ content: `${message.author} You have exceeded the amount of mentions the server has setted!` }).then(msg => {
...
client.on('interactionCreate', async (interaction) => {
    if (interaction.isSelectMenu()) {
        ...
        if (interaction.customId == 'setup_settings') {
          ...
          if(selected == "setup_mmesg"){
            const row = new Discord.MessageActionRow()
                    .addComponents([
                        new Discord.MessageButton()
                        .setLabel(`Setup Anti Mass Mention!`)
                        .setStyle(`SECONDARY`)
                        .setCustomId(`add_mmention`),
                        new Discord.MessageButton()
                        .setLabel(`Disable Anti Mass Mention`)
                        .setStyle(`DANGER`)
                        .setCustomId(`remove_mmention`),
                    ])



                const allReminders2 = await antiMassMention.findOne({
                    Guild: interaction.guild.id
                })
                ...
                interaction.reply({
                    content: `${client.config.emojis.message} Anti Mass Mention System! A customaizable system that makes you able to set up anti mass mention!`,
                    components: [row],
                    ephemeral: true
                })
          }
          
          if(selected == "setup_antialts"){
            const row = new Discord.MessageActionRow()
                    .addComponents([
                        new Discord.MessageButton()
                        .setLabel(`Setup Anti Alt!`)
                        .setStyle(`SECONDARY`)
                        .setCustomId(`add_antialt`),
                        new Discord.MessageButton()
                        .setLabel(`Disable Anti Alt!`)
                        .setStyle(`DANGER`)
                        .setCustomId(`remove_antialt`),
                    ])



                const allReminders2 = await antiAlt.findOne({
                    Guild: interaction.guild.id
                })
                ...
                interaction.reply({
                    content: `${client.config.emojis.message} Anti Alt System! Add layers of difficulty to members who join!`,
                    components: [row],
                    ephemeral: true
                })
          }
          
          if(selected == "setup_antispam"){
            const row = new Discord.MessageActionRow()
                    .addComponents([
                        new Discord.MessageButton()
                        .setLabel(`Setup Anti Spam!`)
                        .setStyle(`SECONDARY`)
                        .setCustomId(`add_antispam`),
                        new Discord.MessageButton()
                        .setLabel(`Disable Anti Spam!`)
                        .setStyle(`DANGER`)
                        .setCustomId(`remove_antispam`),
                    ])



                const allReminders2 = await antiSpam.findOne({
                    Guild: interaction.guild.id
                })
                ...
                interaction.reply({
                    content: `${client.config.emojis.message} Anti Spam! System! Make sure that no one can spam and ruin chat!`,
                    components: [row],
                    ephemeral: true
                })
          }
    ...
    if (!interaction.isCommand()) {
      ...
      if(interaction.customId == "add_mmention"){
        // Setup code (It will just start mass mention code)
        
        const modal = new Modal()
                .setCustomId(`add_mmention`)
                .setTitle(`Setup Anti Mention!`)
                .addComponents([
                    new TextInputComponent()
                    .setCustomId(`amount`)
                    .setLabel('Amount')
                    .setStyle('SHORT') //SHORT' or 'LONG'
                    .setPlaceholder(`Pick a number!`)
                    .setRequired(true),
                 //   new TextInputComponent()
                   // .setCustomId(`action`)
                    //.setLabel('Action')
      ...
      if(interaction.customId == "remove_mmention"){
        // Remove Code (Removes everything related to mass mention)
        await interaction.deferReply({
            ephemeral: true
        });

        const datas = await antiMassMention.find({
        Guild: interaction.guild.id
    })

    if(!datas) return interaction.editReply({
        content: `${client.config.emojis.error} The Anti Mass Mention System Has not been set up yet!`,
        ephemeral: true
    })

        
        datas.forEach(async anti => {
          
          
          const data = await antiMassMention.findOne({
            Guild: interaction.guild.id,
            Amount: anti.Amount
        })
        ...
        interaction.editReply({
            content: `${client.config.emojis.message} The Anti Mass Mention System removal has been Successful!`,
            ephemeral: true
        })
       ...
       if(interaction.customId == "add_antialt"){
        // Setup code (It will just start mass mention code)
        let data = antiAlt.findOne({
            Guild: interaction.guild.id,
        })

         // if(data) return interaction.reply({content: `${client.config.emojis.error} The anti alt system has already been setup!`, ephemeral: true})
        
        const modal = new Modal()
                .setCustomId(`add_antialt`)
                .setTitle(`Setup Anti Alt!!`)
                .addComponents([
                    new TextInputComponent()
      ...
      if(interaction.customId == "remove_antialt"){
        // Remove Code (Removes everything related to mass mention)
        await interaction.deferReply({
            ephemeral: true
        });

        const datas = await antiAlt.find({
        Guild: interaction.guild.id
    })

    if(!datas) return interaction.editReply({
        content: `${client.config.emojis.error} The Anti Alt System Has not been set up yet!`,
        ephemeral: true
    })
        ...
        datas.forEach(async anti => {
          ...
          const data = await antiAlt.findOne({
            Guild: interaction.guild.id,
            Difficulty: anti.Difficulty
        })
        ...
        interaction.editReply({
            content: `${client.config.emojis.message} The Anti Alt Mention System removal has been Successful!`,
            ephemeral: true
        })
      ...
      if(interaction.customId == "add_antispam"){
        const modal = new Modal()
                .setCustomId(`add_antispam`)
                .setTitle(`Setup Anti Spam!`)
                .addComponents([
                    new TextInputComponent()
                    .setCustomId(`amount`)
                    .setLabel('Amount')
                    .setStyle('SHORT') //SHORT' or 'LONG'
                    .setPlaceholder(`Any amount of numbers`)
                    .setRequired(true),
                  new TextInputComponent()
                  .setCustomId(`time`)
                  .setLabel('Time')
                  .setStyle('SHORT')
                  .setPlaceholder(`[1 - 60 seconds]`)
                  .setRequired(true)
                 //   new TextInputComponent()
                   // .setCustomId(`action`)
                    //.setLabel('Action')
      ...
      if(interaction.customId == "remove_antispam"){
        await interaction.deferReply({
            ephemeral: true
        });

        const datas = await antiSpam.find({
        Guild: interaction.guild.id
    })
        ...
        if (!data) return interaction.editReply({
            content: `${client.config.emojis.error} The Anti Spam System Has not been set up yet!`,
            ephemeral: true
        })
        ...
        interaction.editReply({
            content: `${client.config.emojis.message} The Anti Spam Mention System removal has been Successful!`,
            ephemeral: true
        })
                    ...
                    .addComponents([
                        ...
                        .addOptions({
                            ...
                            value: 'setup_anti_word',
                                    ...
                                    {
                          label: 'Setup anti-mass mention',
                          value: 'setup_mmesg',
                        },
                                    
                                    {
                          label: 'Setup anti-spam',
                          value: 'setup_antispam'
                        },
                                    
                                    {
                          label: 'Setup anti-alt',
                          value: 'setup_antialts'
...
client.on('modalSubmit', async (modal, interaction) => {
    ...
    if (modal.customId == 'edit_antiword') {
        ...
        if (action.toUpperCase() == "DELETE") {
            ...
            if (!data) return modal.editReply({
                content: `${client.config.emojis.error} A Keyword message does not exist in this server. Please add a new one and try again!`,
                ephemeral: true
            })
  ...
  if(modal.customId == "add_mmention"){
    await modal.deferReply({
            ephemeral: true
        });
       ...
       const amount = modal.getTextInputValue(`amount`)
    
        if (!amount) return modal.editReply({
            content: `${client.config.emojis.error} Please provide a valid Amount!`,
            ephemeral: true
        })
        ...
        if (data) return modal.editReply({
            content: `${client.config.emojis.error} The Mass Mention system already exists!`,
            ephemeral: true
        })



        await (new antiMassMention({
            Guild: modal.guild.id,
            Amount: amount,
        })).save();

        modal.editReply({
            content: `${client.config.emojis.message} The Mass Mention System has been setup!`,
            ephemeral: true
        })
   ...
   if(modal.customId == "add_antialt"){
    await modal.deferReply({
       ...
       const diffi = modal.getTextInputValue(`difficulty`)
    
        if (!diffi) return modal.editReply({
            content: `${client.config.emojis.error} Please provide a valid Difficulty!!`,
            ephemeral: true
        })
    

        const diffii = parseInt(diffi)

        console.log(diffii)

        if(diffii > 5) return modal.editReply({content: `${client.config.emojis.error} Your number is over the maximum value of 5!`})

        if(diffii < 1) return modal.editReply({content: `${client.config.emojis.error} Your number is over the minimum value of 1!`})


     if(isNaN(diffii)) return modal.editReply({content: `${client.config.emojis.error} Your value is not a number`})
        ...
        if (data) return modal.editReply({
            content: `${client.config.emojis.error} The Anti Alt system already exists!`,
            ephemeral: true
        })
        ...
        modal.editReply({
            content: `${client.config.emojis.message} The Anti Alt System has been setup!`,
            ephemeral: true
        })
  ...
  if(modal.customId == "add_antispam"){
    await modal.deferReply({
            ephemeral: true
        });
     ...
     if (!time || isNaN(ms(time))) return modal.editReply({
                content: "" + client.config.emojis.error + " Incorrect Time string, must be  `s`**. S-seconds",
                ephemeral: true
            });
    
            if (ms(time) > ms("1m")) return modal.editReply({
                content: "" + client.config.emojis.error + " Incorrect time string, must be longer shorter than 1 minute",
                ephemeral: true
            });
    ...
    const ConvertedTime = ms(time);
        ...
        if (data) return modal.editReply({
            content: `${client.config.emojis.error} The Anti Spam system already exists!`,
            ephemeral: true
        })
        ...
        modal.editReply({
            content: `${client.config.emojis.message} The Anti Spam System has been setup!`,
            ephemeral: true
        })
  }
...
function getTimeSolid(args, eph = true, arg1) {
    args = args.join(' ');
    args = ' ' + args;
    console.log("bash ./time.sh" + args);
    try {
